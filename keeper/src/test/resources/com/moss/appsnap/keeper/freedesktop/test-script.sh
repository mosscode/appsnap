...
CONTROL=CONTROL_DIR/socket-control
DIR=CONTROL_DIR

xmessage "Checking for updates..." -buttons "" -center -title "Please Wait..." &
DIALOG=$!

RESPONSE=`echo -n "LAUNCH_POLL" | sendFifoSocketMessage $CONTROL $DIR`
...
if [ "$RESPONSE" != "OK" ] 
then
	xmessage -center -buttons OK -default OK  "Polling error: $RESPONSE"
	exit 1;
fi
